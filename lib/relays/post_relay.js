...
function recievePosts(relay, tube, result) {
  return new Promise((full) => {
    ...
    var items = result.items.sort((a, b) => b.id - a.id)
    ...
    var more = false;
    if (smallesIdNeed.id === smallestId.id && items.length === LIMIT) {
      more = true;
    }
...
function checkPosts(relay, offset, api, tube) {
  return new Promise((full) => {
    ...
    return api.call('wall.get', params);
  }).catch(e => {
    console.log(e);
    if (e.type === errors.ACCESS_DENIED) {
      console.log('skipping');
      return Promise.resolve([]); //skip
    }
    return pause(5000)
      .then(_ => checkPosts(relay, offset, api, tube));
